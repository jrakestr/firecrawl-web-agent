...
- RideCo Ops (`rideco-ops-session`) and Alesig CAF (`alesig-caf-session`) profiles are separate from MyGameSim; CAF export/import into `rideco-reporting` is handled in the sibling repo `~/Development/rideco-data-explorer` via `scripts/import_caf_export.py`.
- `agent-core copy/.env` holds MyGameSim, Rocket Money, RideCo Ops, Alesig CAF, and Supabase settings.
