...
Pass `uploads` to `run()`/`stream()` and the agent can convert documents into clean markdown with the `parse` tool, which calls Firecrawl's [`/v2/parse`](https://docs.firecrawl.dev/features/parse) endpoint.
...
const result = await agent.run({
  ...
  uploads: [
    { name: 'contract.pdf', type: 'application/pdf', content: fs.readFileSync('contract.pdf').toString('base64') },
  ],
})
```

- **Uploaded files** (`.pdf`, `.docx`, `.doc`, `.odt`, `.rtf`, `.xlsx`, `.xls`, `.html`, up to 50 MB): the agent calls `parse` with the file name. Binary files must be base64-encoded in `UploadedFile.content`.
- **Hosted documents** (a public PDF URL): use `scrape` — Firecrawl auto-detects and parses them. No upload needed.
- **Scanned PDFs:** the model sets `mode: 'ocr'` (the default `auto` mode also falls back to OCR). `fast` is text-only.
- **Structured extraction:** the model can pass a JSON `schema` to `parse` to pull fields (e.g. invoice totals, contract parties) directly, returned alongside the markdown.

`createParseTool(apiKey)` is also exported if you want to attach the tool to your own toolkit.
