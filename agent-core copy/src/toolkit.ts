...
export function buildFirecrawlToolkit(
  ...
  // `firecrawl-aisdk` does not wrap the /v2/parse endpoint, so we build the
  // parse tool here and bolt it onto the toolkit. It's gated like the other
  // tools in createFiltered so sub-agents only get it when whitelisted.
  const parseTool = createParseTool(firecrawlApiKey);
  const includeParse = (enabled?: string[]) => !enabled || enabled.includes("parse");

  if (bashMode) {
    const { scrape: _scrape, ...rest } = tools;
    const bashTools = { ...rest, scrapeBash, parse: parseTool };
    ...
    return {
      ...
      createFiltered: (enabled) => {
        ...
        return { ...filtered, scrapeBash, ...(includeParse(enabled) ? { parse: parseTool } : {}) };
      },
    };
  }

  return {
    tools: { ...tools, parse: parseTool } as never,
    systemPrompt: systemPrompt ?? undefined,
    createFiltered: (enabled) => {
      const opts: Record<string, unknown> = {
        apiKey: firecrawlApiKey,
      ...
      return { ...filtered, ...(includeParse(enabled) ? { parse: parseTool } : {}) };
    },
  };
}
