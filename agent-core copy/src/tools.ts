...
// just-bash defaults cap heredocs, string length, and command output at 10MB
// each. Uploaded documents (Firecrawl /parse accepts up to 50MB → ~67MB once
// base64-encoded) blow past that, so the sandbox needs generous size limits.
// Iteration/recursion limits are left at their defaults.
const BASH_LIMITS = {
  maxHeredocSize: 134217728, // 128MB
  maxStringLength: 134217728,
  maxOutputSize: 134217728,
} as const;

async function createBash(): Promise<BashInstance> {
  const { Bash } = await import("just-bash");
  return new Bash({ executionLimits: BASH_LIMITS }) as unknown as BashInstance;
}
...
export async function initBashWithFiles(files: Record<string, string>) {
  ...
  for (const [filePath, content] of Object.entries(files)) {
    ...
    // Write via the fs API, not a heredoc — heredocs are parsed as a single
    // command string and are subject to maxHeredocSize, which large uploads
    // exceed. writeFile stores the exact bytes with no size-limited parsing
    // and no trailing-newline artifact.
    await bash.writeFile(filePath, content);
  }
}
...
export async function readBashFile(filePath: string): Promise<string> {
  const bash = getSharedBash();
  if (!bash) return "";
  // Read via the fs API rather than `cat` — `cat` routes through the shell and
  // is capped by maxOutputSize, which would truncate large uploaded files.
  try {
    return await bash.readFile(filePath);
  } catch {
    return "";
  }
}

export async function writeBashFile(filePath: string, content: string): Promise<boolean> {
  let bash = getSharedBash();
  if (!bash) {
    bash = await createBash();
    setSharedBash(bash);
  }
  try {
    const dir = filePath.replace(/\/[^/]*$/, "");
