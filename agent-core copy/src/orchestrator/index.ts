...
export async function createOrchestrator(options: OrchestratorOptions) {
  ...
  if (uploadDescriptions.length > 0) {
    contextSections.push(`<uploaded_files>\nThe user uploaded files to the bash filesystem:\n${uploadDescriptions.map((d) => `- ${d}`).join("\n")}\nUse bashExec to explore text files (csv, json, txt, etc.): 'head -5 /data/file.txt', 'cat /data/file.json | jq .', 'wc -l /data/file.txt', etc.\nTo read documents (PDF, DOCX, DOC, ODT, RTF, XLSX, XLS, HTML), call the parse tool with the file name (e.g. parse('report.pdf')) — it returns clean markdown. For scanned PDFs set mode: 'ocr'. Pass a schema to parse to pull structured fields directly.\n</uploaded_files>`);
  }
