...
const DATA_TOOLS = new Set([
  ...
  "extract", "crawl", "map", "batchScrape", "parse",
]);
...
Do not use emojis.${schemaSystemLine}`,
  ...
  private async buildAgent(params: RunParams): Promise<DeepAgent> {
    ...
    // Tell the model what the user uploaded — without this, files are staged in
    // /data but the model has no idea they exist, so a prompt like "parse" gets
    // a "which file?" reply instead of a parse call.
    const firstUploadName = uploadDescriptions.length
      ? uploadDescriptions[0].split(" ")[0].replace("/data/", "")
      : "";
    const uploadsContext = uploadDescriptions.length
      ? `<uploaded_files>\nThe user uploaded files to the bash filesystem:\n${uploadDescriptions
          .map((d) => `- ${d}`)
          .join(
            "\n",
          )}\n\nCRITICAL RULE: These are UPLOADED LOCAL FILES, not web pages. NEVER use the scrape or map tools on them — scrape/map are for public http(s) URLs only and will fail on /data paths.\n\n- To read or extract from a document (PDF, DOCX, DOC, ODT, RTF, XLSX, XLS, HTML): call the parse tool. With one document uploaded you can call parse with no arguments; otherwise pass filename (e.g. '${firstUploadName}'). parse returns clean markdown AND saves the full parsed text to /data/<name>.md.\n- When the user wants SPECIFIC FIELDS, JSON, or a CSV out of a document, call parse with a 'schema' describing those fields — it returns compact structured JSON directly (do NOT dump the whole document and try to reshape it). For scanned PDFs set mode: 'ocr'.\n- parse returns only a PREVIEW for large documents and reports 'fullContentSavedTo'. To work with the whole thing (search, slice, summarize, build a CSV), read it from /data with bashExec — e.g. 'wc -l /data/<name>.md', 'sed -n "1,200p" /data/<name>.md', 'grep -in budget /data/<name>.md'. Never claim you only have a reference: the full text is on disk in /data.\n- To explore text/data files (csv, json, txt, etc.): use bashExec, e.g. 'head -5 /data/file.csv', 'cat /data/file.json | jq .'.\n\nWhen the user says things like "parse", "extract", "read", or "summarize" with a document uploaded, call parse FIRST — do not search or scrape.\n</uploaded_files>`
      : "";
    const systemPrompt = [toolkit.systemPrompt, appSystemPrompt, uploadsContext]
      .filter(Boolean)
      .join("\n\n");
